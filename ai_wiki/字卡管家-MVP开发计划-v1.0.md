...
> **执行状态更新（2026-07-26 09:31）**：Day 0 的 0.1～0.5 已全部完成。Day 1、Day 2 代码已合入项目根目录 `main` 并推送 GitHub（提交 `7706785`），27 项自动测试通过，QuickStart 前端残留已清理。微信开发者工具已切换到项目根目录；工作树切换引起的 WXML 文件索引误报已通过重建文件索引、关闭并重新打开项目解决，最新本地加载不再出现 `WXML file not found`，官方自动化连接成功。当前待完成：授权后执行官方预览上传、部署 `syncSettings` / `cardService`，再按“手动录入大 → 首页待复习 +1 → 字卡库可见及筛选正确”完成端到端验收。

> **最新执行状态（2026-07-26）**：Day 4、Day 5 的生产代码和自动测试已在当前项目根目录完成，测试总数增至 60 项并全部通过。已新增 `reviewService`、`subscriptionService`、`sendReminder`，扩展 `syncSettings.saveSettings`，完成复习整轮事务提交、组词 Bottom Sheet、3000 字静态拼音表、设置编辑、订阅额度幂等、定时提醒和首页双 Banner。按本轮方案 B，尚未上传或部署任何新云函数，也未创建定时触发器；真实数据库事务、订阅授权和消息送达仍需用户上传后验收。Day 3 语音录入仍未开发，Day 1 / Day 2 云端部署与端到端验收仍未完成。
...
> **当前状态（2026-07-26）**：🟡 代码与自动测试已完成，待上传 `reviewService` 并完成真实数据库事务联调。复习页已实现进度、三档标记、自动切卡、整轮一次提交、失败保留结果重试和完成庆祝；`word-sheet` 已在复习页与字卡库复用，支持拼音、内置组词和自定义组词。自动测试已覆盖 session 快照、summary、批量更新、越权和事务回滚。
...
> **当前状态（2026-07-26）**：🟡 代码与自动测试已完成，待用户上传 `syncSettings` / `subscriptionService` / `sendReminder`、创建每小时触发器并进行真机订阅消息验收。设置页、复习完成授权卡、额度幂等流水、提醒日志幂等、真实待复习数量、发送成功扣额度/失败不扣额度及首页双 Banner 均已实现；本轮未向微信或腾讯云上传代码。
