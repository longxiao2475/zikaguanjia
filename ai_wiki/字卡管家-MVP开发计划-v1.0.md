...
> **Day 4 / Day 5 联调修复（2026-07-26）**：已修复 `word-sheet` 组件路径导致的拼音/组词点击无响应、首页统计卡无跳转、已学过且从未复习字卡不进入首次计划、认字日仅显示部分按钮和 Day 5 开发提示等问题；字卡库新增汉字/词语片段搜索、多选临时复习队列，选中字卡与今日计划合并去重后复习。拼音搜索延期到后续阶段，不纳入当前 MVP。由于本轮扩展了 `cardService.list/getByIds`，自动测试完成后仍需重新上传部署 `cardService` 并在微信开发者工具完成端到端验收。

> **Day 6 执行状态（2026-07-26）**：✅ 字卡库已补齐编辑内容/熟练度、二次确认软删除、空/加载/错误态；搜索框显式设置输入文字与光标颜色；多选改为 88rpx 热区包裹 38rpx 圆形勾选；普通和多选状态均可点击正文查看详情。`word-sheet` 的开发者工具旧文件索引已重建，最新日志不再出现组件缺失；词语拼音改为逐字读取本地静态表（如“礼物 → lǐ wù”）。首页悬浮按钮和详情关闭按钮也已改为固定尺寸 `view` 控件，避免原生 `button` 被模拟器拉伸。QuickStart、`askDeepSeek`、`aiTask.js` 示例残留已删除，无硬编码密钥扫描命中。自动测试已全部通过；官方自动化可完成路由和截图，但当前开发者工具版本的节点树接口持续阻塞，交互点击仍需在模拟器/真机手动复核。Day 3 语音链路仍未开发，不随 Day 6 误标完成。
...
> **当前状态（2026-07-26）**：🟡 `reviewService` 已上传部署，复习页已实现进度、三档标记、自动切卡、整轮一次提交、失败保留结果重试和完成庆祝；`word-sheet` 的组件路径已在本轮联调中修复，复习页与字卡库均复用拼音、内置组词和自定义组词弹层。自动测试已覆盖 session 快照、summary、批量更新、越权、事务回滚和临时队列合并；仍待开发者工具与真实数据库端到端复核。
...
> **当前状态（2026-07-26）**：✅ 代码、静态拼音/组词、字卡编辑与软删除、视觉修复、示例清理和自动测试完成。开发者工具已重建 fileutils 并正常编译，未再出现 `word-sheet` 组件缺失。⚠️ 官方自动化 SDK 在当前开发者工具版本下只能稳定完成路由与截图，读取 WXML 节点树会阻塞；交互主路径和真机订阅消息仍列为人工验收项。Day 3 语音录入是独立未完成阶段。
