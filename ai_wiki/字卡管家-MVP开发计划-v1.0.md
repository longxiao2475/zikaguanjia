...
> **执行状态更新（2026-07-26 09:31）**：Day 0 的 0.1～0.5 已全部完成。Day 1、Day 2 代码已合入项目根目录 `main` 并推送 GitHub（提交 `7706785`），27 项自动测试通过，QuickStart 前端残留已清理。微信开发者工具已切换到项目根目录；工作树切换引起的 WXML 文件索引误报已通过重建文件索引、关闭并重新打开项目解决，最新本地加载不再出现 `WXML file not found`，官方自动化连接成功。当时待完成：授权后执行官方预览上传、部署 `syncSettings` / `cardService`，再按“手动录入大 → 首页待复习 +1 → 字卡库可见及筛选正确”完成端到端验收。

> **最新执行状态（2026-07-26）**：Day 4、Day 5 的云函数已由用户上传部署，`sendReminder` 的 `reminderCheck` 每小时整点触发器已在腾讯云控制台创建并刷新复核。真实订阅授权与消息送达仍需真机验收；Day 3 语音录入仍未开发。

> **Day 4 / Day 5 联调修复（2026-07-26）**：已修复首页统计卡无跳转、已学过且从未复习字卡不进入首次计划、认字日仅显示部分按钮和 Day 5 开发提示等问题；字卡库新增汉字/词语片段搜索、多选临时复习队列，选中字卡与今日计划合并去重后复习。该轮对 `word-sheet` 路径和文件索引的处理后来仍在真实开发者工具中复现组件缺失，因此最终方案已改为页面内联详情，见下方 Day 6 二次修复。拼音搜索延期到后续阶段，不纳入当前 MVP。由于本轮扩展了 `cardService.list/getByIds`，仍需确认线上 `cardService` 使用最新部署版本。

> **Day 6 执行状态（2026-07-26）**：✅ 字卡库已补齐编辑内容/熟练度、二次确认软删除、空/加载/错误态；搜索框显式设置输入文字与光标颜色；多选圆圈缩小为 38rpx 状态标记；首页悬浮按钮和详情关闭按钮改为固定尺寸 `view` 控件，避免原生 `button` 被模拟器拉伸。QuickStart、`askDeepSeek`、`aiTask.js` 示例残留已删除，无硬编码密钥扫描命中。Day 3 语音链路仍未开发，不随 Day 6 误标完成。

> **Day 6 二次交互修复（2026-07-26）**：✅ 已彻底移除真实开发者工具持续无法解析的 `components/word-sheet`，字卡库和复习页改为页面内联详情，不再依赖自定义组件路径。字卡内容按原顺序拆成不重复单字，例如“妈妈”只显示一个“妈”，“礼物”分别显示“礼”和“物”的本地拼音与各自组词；字母、标点和 emoji 不生成单字分组；补充组词时按目标单字校验，保存一个分组不会清空其他分组的输入草稿。字卡库选择复习改为点击整行选择/取消，圆圈只显示状态。复习页新增“调整顺序”，用原生拖拽仅重排当前及后续字卡，已完成结果不动且顺序只影响本轮。全量自动测试 91/91 通过，JavaScript、页面 JSON 与 diff 静态检查通过；本轮没有修改云函数，无需重新上传云函数。微信开发者工具文件索引已重建，新一轮日志不再出现 `word-sheet` 组件错误；仍需在模拟器或真机手动点击完成最终交互验收。
...
- app.js 已配置 env；写计划时存在的 `cloudfunctions/askDeepSeek` / `utils/aiTask.js` 已在 Day 6 清理（Git 历史可恢复，重新立项后再建设）
...
> **当前状态（2026-07-26 09:31）**：✅ 代码、自动测试及主分支集成已完成。5 个页面、3 个原生 tab、全局设计 token、本地业务缓存与 `syncSettings.bootstrap` 均已落地；开发者工具已从临时工作树切换到项目根目录，WXML 文件索引误报已解决。官方预览上传仍待用户明确授权，云函数部署与业务交互复核归入下方 Day 2 联调收尾。
...
> **当前状态（2026-07-26 09:31）**：🟡 代码、自动测试及主分支集成已完成，云端部署与端到端验收待完成。已实现 `cardService` 分层云函数、前端 API、手动录入、首页缓存优先刷新、今日计划、字卡库三筛选与分页；待授权上传并部署 `syncSettings` / `cardService` 后，验证“录入大 → 首页待复习 +1 → 字卡库可见及筛选正确”。
...
> **当前状态（2026-07-26）**：🟡 `reviewService` 已上传部署，复习页已实现进度、三档标记、自动切卡、整轮一次提交、失败保留结果重试、完成庆祝和本轮待复习字卡拖拽排序。复习页与字卡库均使用页面内联详情，按字卡内容中的不重复单字分别展示本地拼音、内置组词和自定义组词；不再依赖 `word-sheet` 组件。自动测试已覆盖 session 快照、summary、批量更新、越权、事务回滚、临时队列合并和重排后提交对应关系；仍待开发者工具与真实数据库端到端复核。
...
3. 页面内联 bottom sheet：字卡按原顺序拆成不重复单字，每个字分别显示拼音、组词标签和自填组词入口；复习页同时提供本轮待复习字卡拖拽排序
...
> **当前状态（2026-07-26）**：🟡 `syncSettings` / `subscriptionService` / `sendReminder` 已由用户上传部署，`reminderCheck` 每小时整点触发器已创建。设置页、复习完成授权卡、额度幂等流水、提醒日志幂等、真实待复习数量、发送成功扣额度/失败不扣额度及首页双 Banner 均已实现；仍待真机订阅授权和消息送达验收。
...
> **当前状态（2026-07-26）**：✅ 代码、静态拼音/组词、字卡编辑与软删除、整行选择、复习顺序拖拽、视觉修复、示例清理和 91 项自动测试完成。`word-sheet` 自定义组件及页面声明已移除，详情视图直接内联到字卡库和复习页，从架构上消除组件缺失错误。⚠️ 官方自动化 SDK 在当前开发者工具版本下只能稳定完成路由与截图，读取 WXML 节点树会阻塞；交互主路径和真机订阅消息仍列为人工验收项。Day 3 语音录入是独立未完成阶段。

**任务**
1. 字卡库补全：✎ 编辑弹层（改内容/熟练度）、🗑 删除二次确认、整行点按打开内联单字详情、选择模式整行选择/取消
...
- [x] 手动输入增改字卡可用
- [x] 字卡库可筛选全部/待复习/已掌握、可编辑（弹层改内容/熟练度）、可删除（带二次确认）
- [ ] 设认字日+提醒时间→到点收到订阅消息且含今日重点数
- [x] 首页认字日+有待复习时显示醒目提醒 banner（被动兜底）
- [x] 今日待复习清单按优先级正确排序（不熟>一般>熟练，同档越久越靠前）
- [x] 复习后批量标熟练度→上次复习时间/累计次数正确更新
- [x] 点击单字→展示内置词典的常用组词（弹层不跳页）
- [x] 提醒额度≤2 时首页显示预警 banner 引导续订
- [ ] 全程孩子不看屏幕，家长单手可完成主流程
- [ ] 清空本地 Storage 或换机后，登录同一微信可从云端恢复孩子设置、字卡和复习历史
- [x] 订阅消息中的复习数量与云端今日待复习清单一致，不再使用硬编码数量
- [x] 同一提醒任务重复触发不会重复发送或重复扣减额度
...
- [x] 删除 quickstartFunctions 与全部模板残留（同时移除未立项的 askDeepSeek / aiTask 示例）
- [x] 全仓库搜索确认无密钥/模板 ID 以外的硬编码敏感信息
- [x] 6 个核心集合、权限和索引全部配置完成
- [ ] 云函数全部部署最新版（asrProxy / syncSettings / cardService / reviewService / subscriptionService / sendReminder）+ 环境变量就位
- [x] sendReminder 定时触发器已在控制台开启（`reminderCheck`，每小时整点）
